...
def download_and_process(url, statement, type):
    ...
    open(relative_path+"tmp.txt", 'wb').write(r.content)
    # connect to database
    con = sqlite3.connect(relative_path+"database.sqlite")
    cur = con.cursor()
    file = open(relative_path+"tmp.txt",'r', encoding='ISO-8859-15')
    # create table
    ...
    for line in file.readlines():
        if not (line.startswith('//')) and not line.isspace() and not line.startswith("Cruise"): # skip comments, empty lines and heading
            ...
            cur.execute("INSERT INTO CHLA VALUES("+line+");")
            con.commit()
    cur.close()         
    cur.close()
    con.close()
    file.close()
    # cleanup 
    os.remove(relative_path+"tmp.txt")
